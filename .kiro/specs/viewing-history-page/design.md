...
- **RLS**
  ...
  - SELECTポリシー: 本人のみ読取（現状のPublic readは要件に不整合）
...
```
- 既存の`history`ポリシーがPublic readのため、本人のみ読取に変更する。
- `history.movie_id`は`movie.movie_id`と同じ`uuid`型へ移行する（必要に応じて既存データの型変換を実施）。
- `history_id`が手動採番の場合は`generated by default as identity`への移行を検討する。
- ログインユーザー作成時に必ず`list`を作成する運用前提とし、トリガーまたはサインアップ後の初期化処理を追加する。
