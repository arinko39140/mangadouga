...
8. While 閲覧履歴が存在する, the Webアプリケーション shall 各履歴項目（動画）の推しバッジ表示状態を表示する
9. While 閲覧履歴が存在する, the Webアプリケーション shall 各履歴項目（動画）の推し数を表示する
10. While 閲覧履歴が存在する, the Webアプリケーション shall 最終閲覧日時の基準としてクリック日時（clicked_at）を用いる
11. While 閲覧履歴が存在する, the Webアプリケーション shall ログインユーザー本人の履歴のみを表示する
...
4. When 再生領域が存在するページで動画の再生を開始したとき, the Webアプリケーション shall その動画を閲覧として記録する
5. When 再生開始の操作が行われたとき, the Webアプリケーション shall 「再生ボタン押下」を再生開始として扱う
6. When 再生を伴わない操作の場合, the Webアプリケーション shall 閲覧を記録しない（作品ページへの遷移と作品ページでの話数切替は記録対象とする）
7. When 直近30件の閲覧履歴に同一の動画が含まれる場合, the Webアプリケーション shall 新しい履歴を作成しない
