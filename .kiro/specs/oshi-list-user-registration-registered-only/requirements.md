...
## Introduction
本仕様は、推しリストページにおける推し登録をログインユーザーに紐づけ、登録済みのみ表示する振る舞いを定義する。推しリストは「movieの推バッジが済になっているもの」をユーザー単位で管理し、一覧表示する。
...
2. The 推しリスト機能 shall 推しリストの一覧をユーザー単位で管理し、movieの推バッジが済になっているデータのみを表示する
3. While ログインユーザーが未認証のとき, the 推しリスト機能 shall 登録済み推し一覧を表示せずログイン画面へ誘導する
4. If 登録済み推しが0件のとき, then the 推しリスト機能 shall 空状態であることを表示する
5. The 推しリスト機能 shall 他ユーザーに紐づく登録済み推しを表示しない
...
3. When 推しリストページを表示するとき, the 推しリスト機能 shall 推しボタンの初期状態を登録済み（済）として表示する
4. If 推し登録が失敗したとき, then the 推しリスト機能 shall 失敗した旨を表示する
