...
3. When お気に入り推しリストセクションが表示される, the マイページUI shall お気に入り推しリストの概要を表示する（本人閲覧時のみ）
...
3. If お気に入り推しリストが0件である, the マイページUI shall 空状態メッセージを表示する（本人閲覧時のみ）
...
2. When 非公開に設定されたセクションがある, the マイページUI shall 他ユーザー閲覧時に当該セクションの見出しを表示し、内容は非公開である旨（例:「非公開」）を表示する
...
**Objective:** As a ログインユーザー, I want お気に入り推しリストの専用ページを閲覧できる, so that 推しの詳細を閲覧できる

#### Acceptance Criteria
1. The マイページUI shall 本人閲覧時にお気に入り推しリストの専用ページへの導線を表示する
2. When お気に入り推しリストの専用ページが表示される, the 専用ページUI shall 本人の推しリストをグリッド形式で表示する
