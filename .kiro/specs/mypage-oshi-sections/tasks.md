...
- [ ] 1. データ層と可視性の基盤を整える
- [x] 1.1 profile_visibilityの真値化と初期データ整備
...
- [x] 1.3 (P) UserOshiListProviderのサマリー取得を整備する
  - 本人/他ユーザーの閲覧文脈に応じて公開データのみ返す
  - ユーザー未存在や取得失敗の状態を一貫したステータスで返す
  - 空状態の判定に必要な情報を返す
  - _Requirements: 2.1, 3.1, 4.1, 4.4_

- [x] 1.4 (P) UserSeriesProviderのサマリー・一覧・管理操作を整備する
  - サマリーは新着順で最大3件に制限する
...
- [x] 1.5 (P) OshiFavoritesProviderのサマリーと一覧取得を整備する
  - 本人のみ取得できる前提で推しリスト情報を返す
  - サマリーは最大3件程度に制限する
  - _Requirements: 2.3, 6.2_

- [ ] 2. マイページの推しセクションUIを構築する
- [x] 2.1 UserOshiSectionsで順序・可視性・表示条件を統合する
...
- [x] 2.2 UserOshiListPanelで推しリスト概要と導線を表示する
...
- [x] 2.3 UserOshiSeriesPanelで推し作品サマリーと導線を表示する
...
- [x] 3.3 推し作品ページの登録/解除/並べ替え操作を実装する
...
- [x] 4.1 OshiFavoritesPageで本人専用の一覧表示を提供する
  - 未ログイン時はログイン導線へ誘導する
...
- [ ] 5. 統合とテストで要件充足を確認する
- [x] 5.1 セクション遷移と非公開/空状態の統合動作を確認する
