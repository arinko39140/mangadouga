...
**Implementation Notes**
- Integration: 公開状態確認は`list`の`can_display`を参照する
- Validation: 非公開時は`toggleFavorite`をUIで無効化
...
**Implementation Notes**
- Integration: `user_series.can_display=true`を条件に含める
...
- RLSで`list`の公開状態を制御し、非公開時は一覧に出さない
